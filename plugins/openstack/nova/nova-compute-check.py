...
# Nova Agent Status check, doesn't require the nova python client 
...
parser = argparse.ArgumentParser(description='Check OpenStack Nova Compute status')
...
def getComputeState(token):
    tenantID = token[1]
    auth_token = token [0]
    url = args.auth_url + ':8774/v2/' + tenantID + '/os-services'
    ...
    for item in arr:
        if item['state'] == 'down' and item['status'] == 'disabled':
              exit_state = STATE_OK
              state_string = "OK"
              print "Nova Compute status: {state_str}.".format(state_str=state_string)
              sys.exit(exit_state)
              return 0
